...
Used by the unit tests, the evaluation scripts and to build training data
for the learned modulation classifier (:mod:`src.ml`).  All generators use
NumPy's global random state (seed with ``np.random.seed``) and return
``(samples, bits)`` for digital or ``(samples, message)`` for analog
modulations.
"""
...
def add_awgn(signal: np.ndarray, snr_db: float) -> np.ndarray:
    """Add complex white Gaussian noise for the given SNR (over the full band)."""
    if len(signal) == 0:
        return signal
...
def generate_mpsk(
    ...
    """Gray-coded M-PSK, points at k·2π/M.  Returns (samples, bits)."""
    k = int(np.log2(order))
    idx = np.random.randint(0, order, num_symbols)
    gray = np.array([_gray(i) for i in range(order)])
    bits = ((gray[idx][:, None] >> np.arange(k - 1, -1, -1)) & 1).astype(np.uint8).reshape(-1)
    syms = np.exp(2j * np.pi * idx / order)
    sps = int(sample_rate / symbol_rate)
    return _finish(_shape(syms, sps, beta), sample_rate, snr_db, freq_offset_hz), bits
...
def generate_qam(
    ...
    """Square M-QAM with random (not bit-mapped) symbols.  Returns (samples, None)."""
    m = int(np.sqrt(order))
    lv = np.arange(-(m - 1), m, 2, dtype=float)
    s = lv[np.random.randint(0, m, num_symbols)] + 1j * lv[np.random.randint(0, m, num_symbols)]
    s = s / np.sqrt(np.mean(np.abs(s) ** 2))
    sps = int(sample_rate / symbol_rate)
    return _finish(_shape(s, sps, beta), sample_rate, snr_db, freq_offset_hz), None
...
def generate_mfsk(
    ...
    """Continuous-phase M-FSK; adjacent tones are ``mod_index·Rs`` apart.
    Returns (samples, level indices)."""
    lv = np.random.randint(0, order, num_symbols)
    sps = int(sample_rate / symbol_rate)
    freq = np.repeat((lv - (order - 1) / 2.0) * mod_index * symbol_rate, sps)
    sig = np.exp(2j * np.pi * np.cumsum(freq) / sample_rate)
    return _finish(sig, sample_rate, snr_db, freq_offset_hz), lv


def add_phase_noise(signal: np.ndarray, std_deg_per_sample: float) -> np.ndarray:
